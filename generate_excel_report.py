...
# Load the necessary data from the notebook kernel
# This script assumes the notebook has been run and variables are in memory

# For standalone execution, we'll need to access the notebook variables
# We'll use the IPython kernel to access the variables

try:
    # Get variables from IPython kernel if running from notebook context
    from IPython import get_ipython
    ipython = get_ipython()
    
    # Get variables from the notebook namespace
    feature_statistics = ipython.user_ns['feature_statistics']
    dependent_var = ipython.user_ns['dependent_var']
    X_train = ipython.user_ns['X_train']
    X_validation = ipython.user_ns['X_validation']
    cutoff_models = ipython.user_ns['cutoff_models']
    best_models = ipython.user_ns['best_models']
    meta_learner_scores = ipython.user_ns['meta_learner_scores']
    best_meta_learner = ipython.user_ns['best_meta_learner']
    ...
    print(f"Error: {e}")
    print("\nThis script must be run from the notebook context after all cells have been executed.")
    print("Please run this code directly in a notebook cell instead.")
